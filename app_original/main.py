...
def recommend(request: Request, username: str = Form(""), queries: str = Form("")):
    ...
    if queries.strip():
        ...
        # Step 1: 類似度上位5タグの重み設定
        top_5_tags = top_tags[:5]  # 上位5個のタグ
        weights = [1.0, 0.8, 0.6, 0.4, 0.2]  # 順番に重み付け
        tag_weight_map = {tag: weight for tag, weight in zip(top_5_tags, weights)}
        ...
        for pid, v in tag_problems.items():
            ...
            if isinstance(prob_tags, list) and prob_tags:
                # 一致タグの重みの合計を関連度とする
                relevance_score = sum(tag_weight_map.get(tag, 0) for tag in prob_tags)
                ...
                if current_rate is None or diff > current_rate:
                    ...
                    # Add to candidates with relevance score
                    candidates.append((relevance_score, diff, title, url, tag_info))
        
        # Step 3: ランダム選択（関連度上位20件から5件を選択）
        import random
        ...
        # その中からランダムに5件選択
        final_candidates = random.sample(top_20, min(5, len(top_20)))
        ...
        result = [
            ...
            for relevance_score, diff, title, url, tag_info in final_candidates
        ]
        return templates.TemplateResponse("index.html", {"request": request, "result": result, "username": username, "rate": current_rate, "top_tags": top_tags[:5], "queries": queries})
