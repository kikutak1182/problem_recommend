...
class ConfidenceWeights:
    """Weights for Evidence + Inference scoring system"""
    # Inference Score weights (sum to 1.0)
    self_conf: float = 0.60     # Self-confidence weight in inference score
    embed_sim: float = 0.40     # Embedding similarity weight in inference score
    
    # Evidence Score (rule-based): either 1 (match) or 0 (no match)
    rule_based: float = 1.0     # Evidence score: 1 if rule match, 0 otherwise
    
    def __post_init__(self):
        # Inference weights should sum to 1.0
        inference_total = self.self_conf + self.embed_sim
        if not np.isclose(inference_total, 1.0):
            raise ValueError(f"Inference weights (self_conf + embed_sim) must sum to 1.0, got {inference_total}")
...
class ConfidenceSystem:
    ...
    def __init__(self, weights: Optional[ConfidenceWeights] = None):
        ...
        self.keyword_matcher = KeywordMatcher()  # Uses standard tag_definitions.json
...
上記から3つのタグを選び、各タグの確信度(0-1)と根拠を示してください。"""
    ...
    def calculate_composite_confidence(self, problem_title: str, editorial_text: str,
        ...
        for tag_data in self_conf_result["tags"]:
            ...
            # Component 3: Rule-based keyword matching
            rule_based_scores = self.keyword_matcher.calculate_rule_based_scores(
                editorial_text, candidate_tags
            )
            rule_based_score = rule_based_scores.get(tag_id, 0.0)
            
            # Calculate Evidence + Inference Score
            # Step 1: Calculate Inference Score (0.0-1.0 range)
            inference_score = (
                self.weights.self_conf * self_conf +
                self.weights.embed_sim * embed_sim
            )
            
            # Quantize inference score to 0.05 steps
            inference_score = round(inference_score / 0.05) * 0.05
            inference_score = max(0.0, min(1.0, inference_score))
            
            # Step 2: Calculate Evidence Score (1 if rule match, 0 otherwise)
            evidence_score = 1.0 if rule_based_score > 0 else 0.0
            
            # Step 3: Final Score = Evidence Score + Inference Score
            composite_score = evidence_score + inference_score
            ...
            results[tag_id] = {
                ...
                "reasoning": reasoning,
                "scoring_method": "evidence_plus_inference",
                "weights_used": {
                    "inference_weights": {
            ...
            self.logger.info(f"✓ {tag_name}: final={composite_score:.3f} "
                           f"[self={self_conf:.3f}, embed={embed_sim:.3f}, rule={'✓' if rule_based_score > 0 else '✗'}]")
